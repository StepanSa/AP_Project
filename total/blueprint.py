...
import json
from total.models import User, Ticket, Transaction, Session
...
def create_user():
	try:
		...
		if (request.authorization is None or user is None
			...
			return make_response("Only admins can create other admins", 405)
	...
	return make_response("New user was successfully created!", 200)
...
def create_ticket():
	try:
		...
		response = dict({"Error": e.normalized_messages()})
		return response
	return make_response("Ticket created", 200)
