...
"___________________________________________________User___________________________________________________"
...
def get_user_by_id(uid):
	if request.method == "GET":
		...
		if current_identity_username != user.username and current_identity_username != 'admin':
			return make_response('Access is denied', 401)
...
"___________________________________________________Ticket___________________________________________________"
...
"___________________________________________________Transaction___________________________________________________"
...
def update_transaction(TransactionId):
	try:
		...
		if transaction == 400:
			return make_response({f"Transaction {TransactionId} doesn't exist": 405})

		if transaction.userId != user.id:
			return make_response({"The transaction id is not yours": 402})
		if transaction.status == "denied":
			return make_response({"The transaction is already denied": 403})


		order_data = UpdateOrder().load(request.json)

		if order_data["status"] == 'denied':
			...
			db_utils.update_entry(transaction, **order_data)
...
def get_order_by_id(id):
	if request.method == "GET":
		...
		if transaction.userId != user.id and user.username != 'admin':
			return make_response({"The transaction id is not yours": 402})
		if transaction == 400:
			return make_response({"Invalid id": 400})
		return jsonify(GetOrder().dump(transaction))
